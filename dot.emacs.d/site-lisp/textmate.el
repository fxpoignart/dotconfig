...
(setq defaults
      '(
        (?\" . (lambda (num-to-insert) (skeleton-pair-insert-maybe num-to-insert) ))
        (?\' . (lambda (num-to-insert) (skeleton-pair-insert-maybe num-to-insert) ))
        (?\) . (lambda (num-to-insert) (insert-char ?\) 1) ))
        (?\] . (lambda (num-to-insert) (insert-char ?\] 1) ))
        (?\} . (lambda (num-to-insert) (insert-char ?\} 1) ))
        ))
...
(defun move-over (char)
  (if (eq (char-after) char)
    ...
    (funcall (cdr (assoc char defaults)) (num-to-insert char))
    )
)

(defun num-to-insert (char)
  (let ((mode-ignores (concat (symbol-name major-mode) "-ignores")))
    (if (and (boundp (intern mode-ignores))
             (member char (symbol-value (intern mode-ignores))))
        1
      nil)))
